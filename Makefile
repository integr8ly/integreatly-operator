...
test/e2e/rhoam/prow:
	$(OPERATOR_SDK) --verbose test local ./test/e2e --operator-namespace="$(WATCH_NAMESPACE)" --go-test-flags "-timeout=120m" --debug --image=$(MANAGED_API_OPERATOR_IMAGE)
...
.PHONY: test/e2e/regex
test/e2e/regex: export WATCH_NAMESPACE := $(NAMESPACE)
test/e2e/regex:
	go clean -testcache && go test -v ./test/functional -run="$REGEX" -timeout=80m
