...
Measure the downtime of the RHOAM components during RHOAM upgrade.
Verify quota is set correctly.
...
9. Go to OpenShift console search for config map `quota-config-managed-api-service` in redhat-rhoam-operator namespace go to the section where a quota config for 5 million (`"name": "5"`) is defined

10. Compare the values of resources with what you get by running this command from the terminal
