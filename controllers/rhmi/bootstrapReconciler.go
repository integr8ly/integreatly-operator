...
func (r *Reconciler) retrieveConsoleURLAndSubdomain(ctx context.Context, serverClient k8sclient.Client) (integreatlyv1alpha1.StatusPhase, error) {
	...
	customDomainStatus := r.installation.Status.CustomDomain
	routingSubdomain := r.installation.Spec.RoutingSubdomain
	if routingSubdomain != "" {
		if customDomainStatus != nil {
			return updateCustomDomainError(customDomainStatus, err)
		}
		routingSubdomain = getDefaultRoutingSubdomain(consoleRouteCR)
		return integreatlyv1alpha1.PhaseCompleted, nil
	}
	if !ok {
		routingSubdomain = getDefaultRoutingSubdomain(consoleRouteCR)
	...
	routingSubdomain = domain
	customDomainStatus = &integreatlyv1alpha1.CustomDomainStatus{Enabled: true, Error: ""}
	return updateCustomDomainError(customDomainStatus, err)
}
