...
func waitForKeycloakAlertState(expectedState string, ctx *TestingContext, t TestingTB) error {
	err := wait.PollUntilContextTimeout(context.TODO(), monitoringRetryInterval, monitoringTimeout, true, func(ctx2 context.Context) (done bool, err error) {
		err = getKeycloakAlertState(ctx)
		if err != nil {
...
func checkKeycloakOperatorReplicasAreReady(ctx *TestingContext, t TestingTB, originalOperatorReplicas int32) error {
	...
	err := wait.PollUntilContextTimeout(context.TODO(), verifyOperatorDeploymentRetryInterval, verifyOperatorDeploymentTimeout, false, func(ctx2 context.Context) (done bool, err error) {
