...
func TestIntegreatlyCustomerDashboardsExist(t TestingTB, ctx *TestingContext) {
	testIntegreatlyCustomerDashboardsExist(t, ctx, MonitoringOperatorNamespace)
}

func TestIntegreatlyCustomerDashboardsExistObservability(t TestingTB, ctx *TestingContext) {
	testIntegreatlyCustomerDashboardsExist(t, ctx, ObservabilityProductNamespace)
}

func testIntegreatlyCustomerDashboardsExist(t TestingTB, ctx *TestingContext, monitoringNamespace string) {
	...
	if err != nil {
		t.Fatal("failed to exec to pod:", err, "pod name:", prometheusPodName, "container name:", containerName, "namespace:", monitoringNamespace)
	}
...
func TestIntegreatlyMiddelewareDashboardsExist(t TestingTB, ctx *TestingContext) {
	testIntegreatlyMiddelewareDashboardsExist(t, ctx, MonitoringOperatorNamespace)
}

func TestIntegreatlyMiddelewareDashboardsExistObservability(t TestingTB, ctx *TestingContext) {
	testIntegreatlyMiddelewareDashboardsExist(t, ctx, ObservabilityProductNamespace)
}

func testIntegreatlyMiddelewareDashboardsExist(t TestingTB, ctx *TestingContext, monitoringNamespace string) {
	// get console master url
	rhmi, err := GetRHMI(ctx.Client, true)
	if err != nil {
		t.Fatalf("error getting RHMI CR: %v", err)
