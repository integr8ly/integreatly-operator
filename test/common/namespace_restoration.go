...
func waitForProductStageStatusInRHMI(t TestingTB, ctx *TestingContext, stage integreatlyv1alpha1.StageName, status integreatlyv1alpha1.StatusPhase) error {
	err := wait.PollUntilContextTimeout(context.TODO(), stageRetryInterval, stageRestorationTimeOut, false, func(ctx2 context.Context) (done bool, err error) {
		rhmi, err := GetRHMI(ctx.Client, true)
		if err != nil {
...
func removeKeyCloakClientFinalizers(ctx *TestingContext, nameSpace string) error {
	err := wait.PollUntilContextTimeout(context.TODO(), finalizerDeletionRetryInterval, finalizerDeletionTimeout, false, func(ctx2 context.Context) (done bool, err error) {
