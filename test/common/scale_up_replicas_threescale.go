...
func TestReplicasInThreescale(t *testing.T, ctx *TestingContext) {
	...
	if err := checkNumberOfReplicasAgainstValue(apim, ctx, minNumberOfReplicas, retryInterval, timeout, t); err != nil {
	...
	apim, err = updateAPIManager(ctx, minNumberOfReplicas)
	if err != nil {
		t.Fatalf("Unable to update : %v", err)
	}
	...
	if err := check3ScaleReplicasAreReady(ctx, t, minNumberOfReplicas, retryInterval, timeout); err != nil {
