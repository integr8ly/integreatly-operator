...
func JUnitReportXML(report *parser.Report, noXMLHeader bool, goVersion string, w io.Writer) error {
	...
	if len(report.Packages) > 0 {
		...
		suite.Properties = append(suite.Properties, JUnitProperty{"go.version", goVersion})
		if pkg.CoveragePct != "" {
			suite.Properties = append(suite.Properties, JUnitProperty{"coverage.statements.pct", pkg.CoveragePct})
		}
		...
		for _, benchmark := range pkg.Benchmarks {
			...
			suite.TestCases = append(suite.TestCases, benchmarkCase)
		}
