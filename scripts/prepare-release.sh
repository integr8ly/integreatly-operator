...
set_related_images() {
  ...
  do
    ...
    if [[ $(yq e ".products[$i].installType" ./products/products.yaml) == *"rhoam"* && $(yq e ".products[$i].quayScan" ./products/products.yaml ) == true ]]; then
      ...
      if [[ "$product_dir" == *"integreatly-rhsso"* ]]; then
      component_version=$(grep currentCSV manifests/$product_dir/*.package.yaml | awk -F  "operator.v" '{print $2}')
      elif [[ "$product_dir" == *"observability-operator"* ]]; then
