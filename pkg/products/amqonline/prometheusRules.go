...
func (r *Reconciler) reconcileKubeStateMetricsAmqOnline(ctx context.Context, serverClient k8sclient.Client) (integreatlyv1alpha1.StatusPhase, error) {
	...
	rules := []monitoringv1.Rule{
		...
		},
		{
			...
			Expr:   intstr.FromString(fmt.Sprintf("sum by(container, pod) (container_memory_usage_bytes{container!='',namespace='%s'}) / sum by(container, pod) (kube_pod_container_resource_limits_memory_bytes{namespace='%s'}) * 100 > 90", namespace, namespace)),
			For:    "15m",
