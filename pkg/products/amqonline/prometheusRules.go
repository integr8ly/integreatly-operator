				...
				Rules: []monitoringv1.Rule{
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("kube_endpoint_address_available{endpoint='console',namespace='%s'}!=2", r.Config.GetNamespace())),
						For:    "300s",
					...
					},
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("kube_endpoint_address_available{endpoint='standard-authservice',namespace='%s'}!=%v", r.Config.GetNamespace(), keycloakServicePortCount)),
						For:    "300s",
						Labels: map[string]string{"severity": "critical", "product": installationName},
					},
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("kube_pod_status_ready{condition='true',namespace='%s',pod=~'enmasse-operator-.*'}!=1", r.Config.GetNamespace())),
						For:    "300s",
						Labels: map[string]string{"severity": "critical", "product": installationName},
					},
				},
				...
				Rules: []monitoringv1.Rule{
					{
						...
						Expr: intstr.FromString(
							fmt.Sprintf("sum(sum(kube_pod_status_ready{condition='true',namespace='%s'}) by (namespace, pod) * on (namespace, pod) group_left() (sum(kube_pod_status_phase{phase='Running'}) by (pod, namespace) == 1)) < 2", r.Config.GetNamespace())),
						For:    "5m",
						Labels: map[string]string{"severity": "critical", "product": installationName},
					},
					{
