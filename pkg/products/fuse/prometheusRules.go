				...
				Rules: []monitoringv1.Rule{
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("sum(kube_pod_status_ready{condition='true',namespace='%[1]v'}) by (namespace, pod) *  on(pod, namespace) kube_pod_labels{label_deploymentconfig='%[1]v'} * on (namespace, pod) group_left() (sum(kube_pod_status_phase{phase='Running'}) by (pod, namespace) == 1) != 1 or sum(kube_pod_status_ready{condition='true',namespace='%[1]v'} * on (pod, namespace) kube_pod_labels{label_deploymentconfig='syndesis-server'}) < 1", r.Config.GetNamespace())),
						For:    "5m",
						Labels: map[string]string{"severity": "critical", "product": installationName},
					},
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("sum(kube_pod_status_ready{condition='true',namespace='%s'}) by (namespace, pod) * on (namespace, pod) group_left() (sum(kube_pod_status_phase{phase='Running'}) by (pod, namespace) == 1) * on (pod, namespace) kube_pod_labels{label_deploymentconfig='syndesis-server'} != 1", r.Config.GetNamespace())),
						For:    "5m",
						Labels: map[string]string{"severity": "critical", "product": installationName},
					},
				},
