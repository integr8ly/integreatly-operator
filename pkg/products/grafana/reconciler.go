...
const (
	...
	defaultUSerFacingGrafanaClusterRoleName = "grafana-user-cluster-permissions"
...
func (r *Reconciler) Reconcile(ctx context.Context, installation *integreatlyv1alpha1.RHMI, product *integreatlyv1alpha1.RHMIProductStatus, client k8sclient.Client) (integreatlyv1alpha1.StatusPhase, error) {
	...
	phase, err = r.reconcileGrafanaServiceaccountRole(ctx, client)
	if err != nil || phase != integreatlyv1alpha1.PhaseCompleted {
		events.HandleError(r.recorder, installation, phase, "failed to reconcile grafana serviceaccount role", err)
		return phase, err
	}

	phase, err = r.reconcileGrafanaServiceaccountRolebinding(ctx, client)
	if err != nil || phase != integreatlyv1alpha1.PhaseCompleted {
...
func (r *Reconciler) reconcileGrafanaServiceaccountRole(ctx context.Context, serverClient k8sclient.Client) (integreatlyv1alpha1.StatusPhase, error) {
	role := &rbac.ClusterRole{
		ObjectMeta: metav1.ObjectMeta{
			Name:      defaultUSerFacingGrafanaClusterRoleName,
...
func (r *Reconciler) reconcileGrafanaServiceaccountRolebinding(ctx context.Context, serverClient k8sclient.Client) (integreatlyv1alpha1.StatusPhase, error) {
	binding := &rbac.RoleBinding{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("%s-rb", defaultUSerFacingGrafanaClusterRoleName),
			Namespace: r.Config.GetOperatorNamespace(),
		},
	}
