				...
				Rules: []monitoringv1.Rule{
					{
						...
						Expr: intstr.FromString(fmt.Sprintf("sum ( sum(kube_pod_status_ready{condition='true',namespace='%s'}) by (namespace, pod) * on (namespace, pod) group_left() (sum(kube_pod_status_phase{phase='Running'}) by (pod, namespace) == 1) ) <= 2", r.Config.GetNamespace())), For: "5m",
						Labels: map[string]string{"severity": "critical", "product": installationName},
					},
				},
			},
