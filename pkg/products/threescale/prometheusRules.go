...
func (r *Reconciler) reconcileKubeStateMetrics3scaleAlerts(ctx context.Context, serverClient k8sclient.Client) (integreatlyv1alpha1.StatusPhase, error) {
	...
	rules := []monitoringv1.Rule{
		...
		},
		{
			...
			Expr:   intstr.FromString(fmt.Sprintf("sum by(container, pod) (container_memory_usage_bytes{container!='', container!='system-provider', namespace='%[1]v'}) / sum by(container, pod) (kube_pod_container_resource_limits_memory_bytes{namespace='%[1]v'}) * 100 > 90", namespace)),
			For:    "15m",
		...
		},
		{
			...
			Expr:   intstr.FromString(fmt.Sprintf("sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_rate{namespace='%[1]v'}) by (container, pod) / sum(kube_pod_container_resource_limits_cpu_cores{namespace='%[1]v'}) by (container, pod) * 100 > 90", namespace)),
			For:    "15m",
			Labels: map[string]string{"severity": "warning"},
		},
	}
